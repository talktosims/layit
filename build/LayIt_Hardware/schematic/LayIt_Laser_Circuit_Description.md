...
```
                                    ...
                                    │                                 │
  ┌──────────┐   ┌──────────┐      │  SPI Bus    ┌─────────┐        │
  │ USB-C PD │──→│ Reverse  │──→12V│──(GPIO10-13)─│MCP4822  │        │
  │ Trigger  │   │ Polarity │  Rail│             │Dual DAC │        │
  │ Board    │   │ + Fuse   │      │             └────┬────┘        │
  └──────────┘   └──────────┘      │                  │              │
        │                          │          ┌───────▼───────┐      │
        │         ┌────────┐       │          │   TL072       │      │
...
|-----|------|-------|---------|
| J1 | USB-C PD Trigger Board | 12V PD profile | 12V 3A input via USB-C PD negotiation |
...
### Circuit
```
J1 VOUT (+) ──→ D1 Anode ──→ D1 Cathode ──→ F1 ──→ +12V RAIL
J1 GND  (-) ──→ GND RAIL
```

### Design Notes
- USB-C PD trigger board negotiates 12V from any USB-C PD charger (20W+ recommended). The board outputs a steady 12V once PD negotiation completes.
...
| D+ | ESP32 GPIO20 |
| GND | GND rail |
| VBUS | Not connected to power rail (power from USB-C PD trigger board only) |
| CC1 | R3 (5.1K) to GND |
| CC2 | R4 (5.1K) to GND |
...
- VBUS is not used for powering the board (USB-C PD trigger board is the sole power source)
...
|-----|-----|------|---------------|---------|
| J1 | 1 | USB-C PD Trigger Board | 12V PD profile | Power input (negotiates 12V from USB-C PD charger) |
...
- [ ] Consider adding TVS diodes on 12V rail for ESD protection
...
- [ ] Consider adding a power switch between USB-C PD trigger board output and D1
